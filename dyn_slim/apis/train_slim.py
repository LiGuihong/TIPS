...
def validate_slim(model, loader, loss_fn, args, log_suffix='', model_mode='largest', get_flops=True):
    batch_time_m = AverageMeter()
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if get_flops:
                if hasattr(model, 'module'):
                    running_flops = model.module.get_flops()
                else:
                    running_flops = model.get_flops()
                flops_m.update(running_flops, 1)
