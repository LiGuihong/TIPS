...
__all__ = ['DSConv2d', 'DSdwConv2d', 'DSpwConv2d', 'DSBatchNorm2d', 'DSBatchNorm2dSG',\
           'DSConv2dBN','DSdwConv2dBN','DSpwConv2dBN','DSLinear', 'DSAvgPool2d', 'DSAdaptiveAvgPool2d']
...
class DSBatchNorm2dSG(nn.Module):
    def __init__(self,
                 num_features_list,
                 eps=1e-5,
                 momentum=0.1,
                 affine=True,
                 track_running_stats=True):
        super(DSBatchNorm2dSG, self).__init__()
        self.out_channels_list = num_features_list
        self.aux_bn = nn.ModuleList([
            nn.BatchNorm2d(channel, affine=False) for channel in
            self.out_channels_list[:-1]])
...
class DSConv2d(nn.Conv2d):
    def __init__(self,
        ...
        self.flops=0
        self.prune_ratio=-1
        self.weight_ranked = None
    def forward(self, x, mode='largest'):
        
        if self.prune_ratio>0 and mode!='largest':
            if self.prune_ratio==1:
                if not self.in_chn_static:
                    self.running_inc = x.size(1)
                if not self.out_chn_static:
                    self.running_outc = self.out_channels_list[0]
                if self.running_outc==0:
                    self.flops=0
                    return x
                ...
                output = F.conv2d(x,
                                ...
                                self.dilation,
                                self.running_groups)
                self.flops=output.size()[1]*output.size()[2]*output.size()[3]*self.running_inc \
                ...
                self.prev_channel_choice = None
                self.channel_choice = -1
                self.real_out_channels = int(self.prune_ratio*self.out_channels_list[0])
                print(self.real_out_channels)
                # print(x.size(), weight.data.size(),)
                output = F.conv2d(x,
                                weight,
                                bias,
                                self.stride,
                                ...
                                self.dilation,
                                self.running_groups)
                output[:,self.weight_ranked[self.real_out_channels:]]=0
                # output[:, self.real_out_channels:]=0
                self.flops=output.size()[1]*output.size()[2]*output.size()[3]*self.running_inc \
                            *self.kernel_size[0]*self.kernel_size[1]/self.groups
                return output
        ...
        if self.prune_ratio>0 and mode=='largest':
            if self.prune_ratio==1:
                if not self.in_chn_static:
                    self.running_inc = x.size(1)
                if not self.out_chn_static:
                    self.running_outc = self.out_channels_list[-1]
                if self.running_outc==0:
                    self.flops=0
                    return x
                weight = self.weight[:self.running_outc, :self.running_inc]
                ...
                equal_in_c = torch.count_nonzero(torch.sum(torch.abs(x), dim=(0,2,3))).item()
                # print(self.prune_ratio, equal_in_c)
                if not self.in_chn_static:
                    self.running_inc = x.size(1)
                if not self.out_chn_static:
                    self.running_outc = self.out_channels_list[-1]
                ...
                self.prev_channel_choice = None
                self.channel_choice = -1
                self.real_out_channels = int(self.prune_ratio*self.out_channels_list[-1])
                # print(self.real_out_channels)
                # print(x.size(), weight.data.size(),)
                output = F.conv2d(x,
                                weight,
                ...
                output[:,self.weight_ranked[self.real_out_channels:]]=0
                # output[:, self.real_out_channels:]=0
                self.flops=output.size()[2]*output.size()[3]*equal_in_c \
                            *self.kernel_size[0]*self.kernel_size[1]/self.groups*self.real_out_channels 
                return output
                           
        if len(self.out_channels_list)<2:
            self.channel_choice=0
        self.channel_choice = min(self.channel_choice, len(self.out_channels_list)-1)
        # print('channel_choice', self.channel_choice,len(self.out_channels_list))
...
class DSConv2dBN(nn.Conv2d):
    def __init__(self,
        ...
        if not isinstance(out_channels_list, (list, tuple)):
            out_channels_list = [out_channels_list]
        super(DSConv2dBN, self).__init__(
            in_channels=in_channels_list[-1],
            out_channels=out_channels_list[-1],
            kernel_size=kernel_size,
            stride=stride,
        ...
        self.bn = DSBatchNorm2d(out_channels_list, **norm_kwargs) 
...
class DSpwConv2dBN(DSConv2dBN):
    def __init__(self,
                 ...
                 padding_mode='zeros',
                 norm_kwargs=None, ):
        super(DSpwConv2dBN, self).__init__(
            in_channels_list=in_channels_list,
            out_channels_list=out_channels_list,
            kernel_size=1,
            stride=1,
...
class DSdwConv2dBN(DSConv2dBN):
    def __init__(self,
        ...
        super(DSdwConv2dBN, self).__init__(
            in_channels_list=channels_list,
            out_channels_list=channels_list,
            kernel_size=kernel_size,
            stride=stride,
...
class DSLinear(nn.Linear):
    ...
    def forward(self, x):
        if self.mode == 'dynamic':
            ...
            self.flops = self.running_outc*self.running_inc
...
class DSAdaptiveAvgPool2d(nn.AdaptiveAvgPool2d):
    ...
    def forward(self, x):
        if self.mode == 'dynamic':
            ...
            self.running_inc = x.size(1)
        self.flops = self.running_inc*x.size(2)*x.size(3)
