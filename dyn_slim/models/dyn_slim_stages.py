...
from dyn_slim.models.dyn_slim_blocks import DSDepthwiseSeparable, DSInvertedResidual, set_exist_attr, DSInvertedResidualNoskipv2, DSBasicBlockNoskipv2
...
class DSStage(nn.Module):
    def __init__(self, stage_type, in_channels_list, out_channels_list,
        ...
        if stage_type.lower() == 'ir':
        ...
        elif stage_type.lower() == 'mbnv2nsv2':
            self.first_block = DSInvertedResidualNoskipv2(
                in_channels_list, out_channels_list, kernel_size,
                layer_num, stride, dilation, act_layer, noskip,
                exp_ratio, se_ratio, norm_layer, norm_kwargs,
                conv_kwargs, drop_path_rate, bias=bias, has_gate=has_gate)
            residual_blocks = []
            for _ in range(layer_num - 1):
                residual_blocks.append(DSInvertedResidualNoskipv2(
                    out_channels_list, out_channels_list, kernel_size,
                    layer_num, 1, dilation, act_layer, noskip,
                    exp_ratio, se_ratio, norm_layer, norm_kwargs,
                    conv_kwargs, drop_path_rate, bias=bias, has_gate=False))
        elif stage_type.lower() == 'rnnsv2':
            self.first_block = DSBasicBlockNoskipv2(
                in_channels_list, out_channels_list, kernel_size,
                layer_num, stride, dilation, act_layer, noskip,
                exp_ratio, se_ratio, norm_layer, norm_kwargs,
                conv_kwargs, drop_path_rate, bias=bias, has_gate=has_gate)
            residual_blocks = []
            for _ in range(layer_num - 1):
                residual_blocks.append(DSBasicBlockNoskipv2(
                    out_channels_list, out_channels_list, kernel_size,
                    layer_num, 1, dilation, act_layer, noskip,
                    exp_ratio, se_ratio, norm_layer, norm_kwargs,
                    conv_kwargs, drop_path_rate, bias=bias, has_gate=False))
        elif  stage_type.lower() == 'ds':
        ...
        self.all_blk = [self.first_block]+residual_blocks
